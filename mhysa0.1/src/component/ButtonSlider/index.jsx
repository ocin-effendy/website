...
function ButtonSlider({ onClick1, onClick2, value }) {
  return (
    <div className="flex">
        <button className='bg-blue-one border border-solid border-blue-one rounded-sm medium:rounded-md w-3 h-3 exsmall:w-5 exsmall:h-5 medium:w-8 medium:h-8 large:w-10 large:h-10 mr-1 large:mr-2.5 flex justify-center items-center text-center active:border-2 active:border-white' onClick={onClick2}><img src={chevron_left} alt="chevron" /></button>
        <input className='border border-solid border-blue-one rounded-sm medium:rounded-md w-3 h-3 exsmall:w-5 exsmall:h-5 medium:w-8 medium:h-8 large:w-10 large:h-10 mr-1 large:mr-2.5 flex justify-center items-center text-center text-suxs exsmall:text-exsm medium:text-sm large:text-base' type="text" value={value} disabled />
