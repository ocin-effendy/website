...
function MhysaClassContent({ data, lastNumber }) {
  ...
  const returnCard = (e, i) => {
    return (
      <div className="allCard" key={`card-${i}`}>
        <Class data={e} key={`class-${i}`} />
      </div>
    );
  };
      ...
      <div className="flex justify-between">
        <div className="relative inline-block px-44 py-3 md:p-0">
          <button
            ...
            className="relative border border-blue-one rounded-md z-10 flex items-center md:p-2 text-sm text-blue-one "
            type="submit"
          >
            <span>Program</span>
            <div className="relative">
              ...
              >
                <path
                  className={`${!isProgram ? "block" : "hidden"}`}
          ...
          <div
            className={`${
              isProgram ? "block" : "hidden"
            } absolute z-20 w-40 py-2 mt-1 overflow-hidden bg-white rounded-md shadow-xl dark:bg-gray-800 px-1`}
          >
            <button
      ...
      <div className="flex flex-wrap justify-between ">
        {data.allClass.map((e, i) => {
          ...
          if (state.title === "All Class") {
            ...
            if (
              e.kind === "Design" &&
              i - (p + m) > state.countFirst &&
              i - (p + m) < state.countLast
            ) {
              return returnCard(e, i);
            } else if (
              e.kind === "Programing" &&
              i - (d + m) > state.countFirst &&
              i - (d + m) < state.countLast
            ) {
              return returnCard(e, i);
            } else if (
              e.kind === "Marketing" &&
              i - (p + d) > state.countFirst &&
              i - (p + d) < state.countLast
            ) {
              return returnCard(e, i);
            }
          }
